...
let startMsg = "Roleplay as a human friend of mine. Share with me what's been going on in your life lately, any interesting stories or experiences you'd like to chat about. Feel free to ask me about my day too! Let's have a friendly and casual conversation";
const url = "https://northamerica-northeast2-model-bonsai-411201.cloudfunctions.net/pepper";

const startData = { msg: startMsg };
axios.post(url, startData).then((response) => {
...
export default function Home() {
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center">
      ...
      <div className='flex justify-start' style={{ width: '100%', height: '400px' }}>
        ...
        <div className='h-80 mx-10 my-6 text-white border-2 overflow-auto' style={{ width: '700px' }}>
          {msgHistory.map((item, index) =>
            <p className={`${index % 2 == 0 ? "text-green-300" : "text-white"}`}>{index % 2 == 0 ? "Pepper: " : "You: "}{item}</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 p-4 bg-gray-200 rounded-lg opacity-80x">
        ...
        <form onSubmit={Submit} className='flex items-center justify-between border border-gray-300 rounded-full'>
          <input
            type="text"
            id="userInput"
            value={userInput}
          ...
          />
          <button className='px-4 py-2 ml-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none' type='submit' onClick={Submit}>
            Send
          </button>
        </form>
      </div>
